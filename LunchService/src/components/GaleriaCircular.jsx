...
class App {
    ...
    onResize() {
        ...
        this.viewport = {
            width: viewHeight * this.camera.aspect,
            height: viewHeight
        };

        if (this.medias) {
            this.medias.forEach((m) =>
                m.onResize({ viewport: this.viewport })
            );
        }
    }
...
export default function GaleriaCircular({ items, bend = 1.5 }) {
    const ref = useRef();
